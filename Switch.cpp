...
extern bool Sec27ASetState;     //   LOW = ??   HIGH = ??
...
Switch::Switch(String alexaInvokeName, unsigned int port, CallbackFunction oncb, CallbackFunction offcb){
    // ESP32 uses a 64-bit MAC address instead of a 32-bit Chip ID
    uint64_t chipId = ESP.getEfuseMac();
    char uuid[64];
    
    // Extract the lower 3 bytes from the MAC address to fill the UUID
...
//  Serial.println(request);
  ...
  if(request.indexOf("GetBinaryState") >= 0) {
    ...
    switchStatus = !Sec27ASetState; 
    
      if (Sec27ASetState == LOW) {
              Serial.println("Sec27ASetState State is LOW (Alarm set))");
     }  

          if (Sec27ASetState == HIGH) {
              Serial.println("Sec27ASetState is HIGH (Alarm Unset))");
     } 

      if (switchStatus == LOW) {
              Serial.println("switchStatus reported to sendRelayState is LOW)");
     }  

          if (switchStatus == HIGH) {
              Serial.println("switchStatus reported to sendRelayState is HIGH)");
     } 
...
//  Serial.println(senderPort);
  ...
  //UDP.write(response.c_str()); 8266 style
